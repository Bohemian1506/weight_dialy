# Day 9-1 開発ログ (2026-05-07 午前の部、4 モード運用初日 + 6 PR / 4 close + Tier 2 全更地化 + 道具選定シリーズ 4 例目完成)

> **Day 9 構成**: 午前の部 (= 本ファイル day-9-1.md) → 午後 ~18:00 (= [day-9-2.md](day-9-2.md)) → 18:00 以降 (= day-9-3.md、後の別 PR で作成予定)

Day 8 (= 5/6) 発表会後夜セッションで導入した「**4 モード切替型運用** (= 学習 / 新規機能 / リファクタ / 調査・計画)」 の **実運用初日・午前の部**。深夜帯のセッション 1 で `CalorieEquivalentService` の enum 化提案を構造ミスマッチで棄却 → ロジック / データ別ファイル分離 (PR #226) で完走。日中のセッション 2 では **ダッシュボード Tier 2 候補** (= `:iphone_with_data` 状態名のリネーム、Issue #161、Tier 2 旧 #5 = 後段の番号繰上げ前) を着手し、Issue 第一候補 `:user_with_data` を採用せず判定実態と一致する **`:has_data`** に変更 (PR #230) → 3 者並列レビュー全 Approve で完走。続くセッション 3 では **PR #95 由来の design-reviewer フォローアップ** (= 「唐揚げ 3 個 5 皿ぶん」 複合読み問題、Issue #99) に着手、`name + unit + 1 単位 kcal` の責務分離リファクタ (PR #232) で完走。続くセッション 4 では **ユーザー局長提案「concern 化はどうか?」 を構造的に検証して見送り判断**、`SessionsController` の 3 行重複を private method に抽出 (PR #237、Issue #228) → **「軽量 Issue は 3 者レビュー指摘を 1 PR で完結」 運用ルール初運用**。続くセッション 5 では **ダッシュボード Tier 2 候補 (= `CalorieAdviceService` の `body` フィールド整理)** に着手、`Result` Struct から `body` を分離して `ZeroKcalResult` を新設 (PR #239) → **「特殊ステートの表現方法」 を 4 段階判断軸として教材化 (= 学び 32)**。**午前締めのセッション 6** では **ダッシュボード Tier 2 最終 1 件 (= `webhooks_controller.rb` の Service 切り出し)** に着手、ドメインロジックを `WebhookHealthDataIngestService` に切り出して controller 181 → 99 行 (-45%) にスリム化 (PR #241) → **「fat controller の Service 切り出し判断軸」 を学び 33 として教材化 + 「残置側にこそ判断ロジックを永続コメントで残す」 という strategic-reviewer 提唱を学び 33 核心実例として反映**。

**午前の部達成サマリ**: **6 リファクタ PR 完走 + ダッシュボード Tier 2 全 3 件完走 (= 更地化) + 道具選定の構造化シリーズ 4 例目 (= 学び 27 enum / 学び 30 Concern / 学び 32 特殊ステート / 学び 33 fat controller) 確立 + 「マージごと dev-log」 / 「軽量 Issue 1 PR 完結」 / 「dev-log も 3 者レビュー」 の 3 つの運用ルール確立**。
...
> **戦略的意義 (= Day 9 reference 日の北極星)**: Day 9 午前で Tier 2 を 1 セッション群で更地化したことは、Day 8 で導入した **リファクタダッシュボード運用** (= `docs/refactor-candidates.md`) **が機能していることの実証**。Day 10 以降の polish 日は「ダッシュボード優先 (= **Tier 1 大物に進む**)」 でモード起動するのが筋 (= 軽い Issue から潰す方針 + ダッシュボード経由フローの再現性確保)。
...
### 起票した Tier 3 候補 Issue (= 1 件、Day 9 セッション 6 由来、午後の day-9-2 で完走)

- **#242** (= `webhook_health_data_ingest_service_spec.rb` 追加 = service unit spec 独立化、PR #241 code-reviewer + strategic-reviewer 共通🟢 提案を別 Issue 起票で吸収) — **午後 day-9-2 セッション 7 で close**

---

## 🧠 教訓ハイライト (= 7 件本日午前確立)

> **道具選定シリーズ (= 学び 27 / 30 / 32 / 33)**: 「使ってみたい衝動を構造軸で機械的に検証する」 共通パターンの 4 例目で完成形。
> **運用ルール系 (= 学び 28 / 29 / 31)**: dev-log / 3 層線引き / grep 運用 / 軽量 Issue 1 PR 完結 等の運用知。
> 学び 33 はさらに午後 day-9-2 の **学び 34** で「テスタビリティ軸 × タイミング軸」 として精緻化される (= シリーズの 1 軸を縦深堀り)。
...
- 観点 ④ (= テスタビリティ) は Concern の隠れた採用動機。独立 unit test を書きたい時は Concern にする筋がある (= 4 観点で唯一 Concern 寄りに振れる軸、午後 day-9-2 の **学び 34** でこの軸を「タイミング × 動機」 で精緻化)
...
#### Day 9 道具選定の構造化シリーズにおける本学びの位置づけ (= 本学びが 3 例目、4 例目は学び 33)

学び 27 (= enum vs ファイル分け) + 学び 30 (= Concern vs private 4 観点) + 本学び 32 (= **3 例目**、特殊ステート) + 学び 33 (= 4 例目、午前締めセッション 6 で fat controller) で **4 例完成**。Day 9 で集中的に蓄積し判断テンプレが固まったマイルストーン。Day 10 以降の道具選定は同じ枠で記録する (= 後述の How to apply 末尾参照)。
...
- **「同じ枠で考える」 の運用ルール (= シリーズ 5 例目以降の書き方)**: Day 10 以降に新しい道具選定が出てきたら、**「使ってみたい衝動を構造軸で検証する枠」** (= 学び 27/30/32/33 と同フォーマット) で day-N の学び N に記録する。具体的には ① 判断対象 + 判定軸を 1 行で言語化、② 段階 / 道具 / 適用条件の table、③ 本 PR の判定 (= ❌ / ✅)、④ 共通パターン参照、の 4 セクション構造。後輩が読んだ時「自分もこのパターンで書ける」 と感じられる粒度を狙う。
...
| ④ テスタビリティ | request spec で十分か / unit test を独立させたいか | 既存 webhook request spec 110 examples で網羅、unit spec は将来別経路追加時に検討 (Issue #242 → 午後 day-9-2 で前倒し追加、学び 34 で 4 軸 × 2 タイミングに精緻化) |
...
- request spec で網羅できるなら **unit spec を機械的に追加しない** (= 「Service 切り出し = 即 unit spec 追加」 のアンチパターン回避)。**ただし** 動機軸が変わったら判断更新 (= ② 契約のドキュメント化 / ③ 一貫性 動機なら前倒し、午後の day-9-2 学び 34 で精緻化、Issue #242 → PR #244 で実証)
...
> **→ 学び 33 のテスタビリティ軸の精緻化は午後 day-9-2 の [学び 34](day-9-2.md#学び-34) を参照** (= 4 軸 × 2 タイミング判定マトリクス、判断の前提が変わったら判断を更新する)
...
- 起票した Issue: **6 件** (= 即日 close 2 件 [#225 + #228] + open 残 3 件 [v1.1 polish 提案 #234 / #235 / #236] + Tier 3 候補 #242 (= 午後 day-9-2 で close))
- ダッシュボード Tier 2: 開始時 3 件 → **終了時 0 件 (= 全 3 件完走、Tier 2 更地化)**
- spec: **585 examples** (= 557 → 560 (+3 by PR #232 食品データ再設計) → 580 (+20 by PR #239 ZeroKcalResult 経路 spec 追加) → 585 (+5 PR #239 追加コミット 1442d9e)、PR #237/#241 は spec 無修正)
- rubocop: **0 offenses**
- 教訓: **7 件** (= 学び 27 + 28 + 29 + 30 + 31 + 32 + 33)
- つまずき / 学び: **2 件**
- セッション時間: **6 セッション** (= セッション 1: 0:00 過ぎ〜4:00 深夜帯 PR #226 / セッション 2: 日中 PR #230 / セッション 3: 日中後半 PR #232 / セッション 4: 日中末 PR #237 / セッション 5: 日中末追加 PR #239 / セッション 6: 午前締め PR #241)
...
- [ ] **Tier 3 候補 #242** (= webhook service spec 追加) — 午後 day-9-2 セッション 7 で完走予定

### 中期 (= Day 10 以降の北極星)
- [ ] **Tier 1 大物 3 件** (= Phase 3 三重防衛 / Capacitor OAuth ブリッジ / daisyUI 全置換) — Day 10 以降の polish 日で着手
...
- **次セッション (= 午後 day-9-2)**: PR #241 で起票した派生 Issue #242 (= webhook service unit spec) を follow-up 着地、軽量 Issue 1 PR ルール 3 例目を実証 (= 学び 31 継承 + 学び 33 テスタビリティ軸の精緻化 = **学び 34** 確立)
- **Day 10 以降**: モード宣言で再開地点を指定 (= Day 8 で確立したフロー)
  - 「今日は学習」 → `project_ruby_basics_learning_track.md` から Lesson 7 候補を提示
  - 「今日はリファクタ」 → `docs/refactor-candidates.md` の **Tier 1 大物 3 件** から 1 つ選ぶ
...
> **→ 続く: [day-9-2.md](day-9-2.md)** (= 午後の部 ~18:00、派生 Issue #242 follow-up 着地 + 学び 34 軸精緻化 + Tier 3 更地化)
