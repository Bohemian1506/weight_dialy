...
memory `feedback_session_mode_declaration.md` で確立した 4 モード運用を機械化するコマンド (= Issue #258、第一群 4 連発の 4 番目 = 最終)。

セッション冒頭の手作業 (= 4 モードごとに散らばった再開地点ソースを横断検索して提示) を 1 コマンドで完結させる。
...
**SSOT (= Single Source of Truth)**: 上記対応表と memory `feedback_session_mode_declaration.md` がズレた場合は **memory が正** (= Day 11 strategic 論点 2 で確立した運用原則、第一群全 Command/Skill 共通)。
...
🎓 学習 (= memory `project_ruby_basics_learning_track.md`)
  Lesson 1-6 完了 / 次 Lesson 7 候補 9 個
...
```bash
# refactor-candidates.md の読み出し
cat docs/refactor-candidates.md
```
