---
name: propose-issue
description: ユーザーが「次にやりたい」「これも対応したい」「これは別 Issue で」 など **未着手タスクへの言及** をした時に、`/plan-issue` での Issue 化を **提案するだけ** する。即起動はしない、ユーザー Yes で Slash Command `/plan-issue` 起動の二段構え。軽微 PR / 質問雑談 / 既着手タスクの内部対話では発火しない。
---
...
**提案文例**:
> 「polish 系の Issue として `/plan-issue` で起票しますか? (= まとめて後でポリッシュ予定なら Backlog 化はせず、独立 Issue で日付込み)」
